...
//double *L,*B,*G,*GL,*GL_T,*F,*Phi,*OMEGA_L,*N_OMEGA_L,*N_OMEGA_L_T,*QQ,*Q_temp;
////Constant in online computing.
...
//void mpc_SP(double *delta_u, double *delta_u_ini,double *y_ini,double *lambda_ini, double *u_k, double *y_k,double *x_k,double *r_k, double *u_k_1, double *u_k_2)
//{
//
//	int i;
//	double WarmVal = 0.5;
...
//	omega_r_form2(aug_u_k_1, x_k, n_omega_r,F);
//	vec_rev(n_omega_r,4*nu*M,n_omega_r);
...
//	//for(i=0;i<ndec;i++)
//	//	c[i] = c[i]*0.00001;
//	vec_rev(c,ndec,c_rev);
//	//printf("c is:\n");show_matrix(c,1,ndec);putchar('\n');
//	//printf("c_rev is:\n");show_matrix(c_rev,1,ndec);putchar('\n');
//
...
//	IPM_V3_DP(G,GL,GL_T,c, N_OMEGA_L,N_OMEGA_L_T, n_omega_r, delta_u_ini, y_ini, lambda_ini, 4*nu*M, nu*M, delta_u_M);
//	//priduip_v4_DP(G, GL,GL_T, c, N_OMEGA_L, N_OMEGA_L_T, n_omega_r, delta_u_ini, y_ini, lambda_ini, 4*nu*M, nu*M, delta_u_M);
...
//	//I do not use vec_add here because I define a #pragma MUST_ITERATE (3) for it!
//	for(i=0;i<nu;i++)
//	{
...
//			j++;
//		}
//		//printf("L is: %u\n");show_matrix(L,1,(nx+ny)*ny);putchar('\n');
//
//		//Below for the QQ
...
//		OMEGA_L_form2(B,Phi,N_OMEGA_L);
//		vec_rev(N_OMEGA_L,(4*nu*M)*nu*M,N_OMEGA_L);
...
//			fprintf(fp_y,"%f\n",y_k[0]);
//			if(i<100) {
//				IAE += scalar_abs(y_k[0] - r_k1[0]);
//				ITAE += i * 0.1 * scalar_abs(y_k[0] - r_k1[0]);
//				ISE += (y_k[0] - r_k1[0]) * (y_k[0] - r_k1[0]);
//			}
//			else {
//				IAE += scalar_abs(y_k[0] - r_k2[0]);
//				ITAE += i * 0.1 * scalar_abs(y_k[0] - r_k2[0]);
//				ISE += (y_k[0] - r_k2[0]) * (y_k[0] - r_k2[0]);
//			}
//
//			fprintf(fp_t,"%f\n",Time_QP/freq.lo*1000);
//			//printf("QP_Iteration: %d\n",QP_Iteration);
